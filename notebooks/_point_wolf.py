...
class PointWOLF(object):
    def __init__(
        self,
        w_num_anchor: int = 4,
        w_sample_type: str = "fps",
        w_sigma: float = 0.5,
        w_R_range: float = 10,
        w_S_range: float = 3,
        w_T_range: float = 0.25,
    ):
        self.num_anchor = w_num_anchor
        self.sample_type = w_sample_type
        self.sigma = w_sigma

        self.R_range = (-abs(w_R_range), abs(w_R_range))
        self.S_range = (1.0, w_S_range)
    ...
    def kernel_regression(self, pos, pos_anchor, pos_transformed):
        ...
        sub = np.expand_dims(pos_anchor, 1).repeat(N, axis=1) - np.expand_dims(
            pos, 0
        ).repeat(
            M, axis=0
        )  # (M,N,3), d
        ...
        pos_new = (np.expand_dims(weight, 2).repeat(3, axis=-1) * pos_transformed).sum(
            0
        )  # (N,3)
    ...
    def local_transformaton(self, pos_normalize):
        ...
        degree = (
            np.pi
            * np.random.uniform(*self.R_range, size=(M, 3))
            / 180.0
            * transformation_dropout[:, 0:1]
        )  # (M,3), sampling from (-R_range, R_range)

        scale = (
            np.random.uniform(*self.S_range, size=(M, 3))
        ...
        trl = (
            np.random.uniform(*self.T_range, size=(M, 3))
            * transformation_dropout[:, 2:3]
        )  # (M,3), sampling from (1, T_range)
        trl = trl * transformation_axis

        # Scaling Matrix
        S = np.expand_dims(scale, axis=1) * np.eye(
            3
        )  # scailing factor to diagonal matrix (M,3) -> (M,3,3)
        # Rotation Matrix
        sin = np.sin(degree)
        cos = np.cos(degree)
